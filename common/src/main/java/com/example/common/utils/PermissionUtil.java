...
/**
 * Created by huangweishui on 2018/5/3.
 */
...
public class PermissionUtil {
    ...
    public static void requestPermisions(Activity activity,
        ...
        if (activity != null && permissions != null && permissions.length != 0 && listener !=
                null) {
            if (!isOverMarshmallow()) {
                ...
                if (denied.size() > 0) {
                    doRequestPermisions(activity, requestCode, denied.toArray(new String[denied
                            .size()]), listener);
                } else {
                    listener.onRequestPermissionSuccess();
                }
